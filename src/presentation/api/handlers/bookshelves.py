...
async def get_bookshelf_list_api_view(
    query_params: Annotated[BookshelfFilter, Depends(books_query_params)],
    current_user: Annotated[UserDTO | None, Depends(get_user_or_none)],
    bookshelf_handler: Annotated[BookshelfQueryHandler, Depends(get_bookshelf_query_handler)],
):
...
async def create_bookshelf_api_view(
    ...
    handler: Annotated[BookshelfCommandHandler, Depends(get_bookshelf_command_handler)],
):
...
@router.put("/{bookshelf_id}", response_model=BookshelfSchema)
