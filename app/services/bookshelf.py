...
from ..orm.query_formats import filter_bookshelf_by_user
...
def _filter_bookshelves_query_by_params(query: _QT, query_params: QueryParams) -> _QT:
    """
    ...
    if query_params["private"] is not None:
        if query_params["private"]:
            query = query.where(Bookshelf.private == True)  # noqa
        else:
            query = query.where(Bookshelf.private == False)  # noqa
    return query
...
async def _get_paginated_bookshelves(
    ...
    bookshelves = [_get_bookshelf_schema(row) for row in res]
...
async def get_bookshelf(session: AsyncSession, bookshelf_id: int, user_id: int) -> BookshelfSchema:
    """
    ...
    query = filter_bookshelf_by_user(query, user_id)
...
async def get_filtered_bookshelves(session: AsyncSession, query_params: QueryParams, user_id: int):
    """
    ...
    query = _filter_bookshelves_query_by_params(query, query_params)
    query = filter_bookshelf_by_user(query, user_id)
...
async def create_bookshelf(
    ...
    bookshelf = Bookshelf(
        ...
        private=bookshelf_schema.private,
        user_id=user_id,
        books=books,
    )
...
async def update_bookshelf(
    ...
    bookshelf.private = bookshelf_schema.private
